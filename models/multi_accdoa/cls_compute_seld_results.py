...
class ComputeSELDResults(object):
    def __init__(
        ...
        for split in os.listdir(self._desc_dir):      
            for ref_file in os.listdir(os.path.join(self._desc_dir, split)):
                ...
                # TAU2020 recordings are 60 s: score the complete recording,
                # including silent tails and predictions after the last GT event.
                # max(gt.keys()) is a frame INDEX, not a recording length, and
                # can also drop the last GT block when that index is a multiple of 10.
                nb_ref_frames = int(round(params['max_audio_len_s'] / params['label_hop_len_s']))
        ...
        self.metric_protocol = 'dcase2023_{}_full_duration'.format(self._average)
    ...
    def get_SELD_Results(self, pred_files_path, is_jackknife=False):
        ...
        if is_jackknife:
            # Keep the full-dataset point estimates. The leave-one-out loop below
            # reuses ER/F/LE/LR variables and must not replace these values.
            global_metric_values = (ER, F, LE, LR, seld_scr)
            global_classwise_results = classwise_results.copy()
            ...
            ER, F, LE, LR, seld_scr = global_metric_values
            classwise_results = global_classwise_results
