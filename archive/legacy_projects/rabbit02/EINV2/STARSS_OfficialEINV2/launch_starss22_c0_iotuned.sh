...
run_id=STARSS22_C0_RealOnly_CkptFix_IOtuned_seed2026
...
timeout 12h "$python_bin" code/main.py \
  -c configs/ein_seld/seld_starss22_c0_realonly_ckptfix_iotuned.yaml \
  --dataset STARSS22 train --seed 2026 --num_workers 2 --port 12447 \
  >"$log_path" 2>&1
status=$?
printf '%s\n' "$status" >"$exit_path"
exit "$status"
