...
class Features_Extractor_MIC():
    def __init__(self, cfg):
        ...
        if cfg['data']['audio_feature'] == 'salsalite':
            ...
            self.lower_bin = int(np.floor(self.fmin_doa * self.n_fft / float(self.fs)))
            self.lower_bin = np.max((self.lower_bin, 1))
            self.upper_bin = int(np.floor(self.fmax_spectra * self.n_fft / float(self.fs)))
            self.cutoff_bin = int(np.floor(self.fmax_spectra * self.n_fft / float(self.fs)))
