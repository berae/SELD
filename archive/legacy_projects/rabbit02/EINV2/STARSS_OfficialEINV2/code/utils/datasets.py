...
class dacase2022_dask3:
    ...
    def __init__(self, root_dir, cfg, args):
        ...
        self.starss23_dir = self.root_dir.joinpath('STARSS23')
        self.real_dataset_name = args.dataset if args.dataset in ['STARSS22', 'STARSS23'] else 'STARSS22'
        ...
        self.dataset_dir['dev']['foa']['STARSS23'] = \
            [self.starss23_dir.joinpath('foa_dev').joinpath('dev-train-sony'),
             self.starss23_dir.joinpath('foa_dev').joinpath('dev-train-tau'),
             self.starss23_dir.joinpath('foa_dev').joinpath('dev-test-sony'),
             self.starss23_dir.joinpath('foa_dev').joinpath('dev-test-tau')]
        self.dataset_dir['dev']['mic']['STARSS23'] = []
        self.dataset_dir['dev']['meta']['STARSS23'] = \
            [self.starss23_dir.joinpath('metadata_dev').joinpath('dev-train-sony'),
             self.starss23_dir.joinpath('metadata_dev').joinpath('dev-train-tau'),
             self.starss23_dir.joinpath('metadata_dev').joinpath('dev-test-sony'),
             self.starss23_dir.joinpath('metadata_dev').joinpath('dev-test-tau')]
        self.dataset_dir['eval'] = {
            'foa':  {
                'STARSS22': [self.starss22_dir.joinpath('foa_eval')],
                'STARSS23': [self.starss23_dir.joinpath('foa_eval')],
            },
            'mic':  { 'STARSS22': [], 'STARSS23': [] },
            'meta': { 'STARSS22': [], 'STARSS23': [] },
            }

        if args.dataset not in ['STARSS22', 'STARSS23']:
