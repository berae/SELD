"""Finite same-host C3 JEPA-weight pair; validation only, no automatic retry."""
import argparse
...
def main():
    ...
    jobs = queue.Queue()
    rows = []
    for label, weight in (('020', .2), ('005', .05)):
        base_id = 'C3_jepa{}_seed2026_rb05_v1'.format(label)
        row = dict(id=('smoke_' if args.smoke else '') + base_id, base_id=base_id,
                   seed=2026, variant='C3', lambda_jepa=weight, status='pending')
        rows.append(row)
        jobs.put(row)
    lock = threading.Lock()
    ...
    def worker(gpu):
        while not stop.is_set():
            ...
            train = [sys.executable, str(ROOT / 'scripts/train/einv2_audited.py'), '--project-root', str(args.project_root),
                     ...
                     '--hdf5-dir', str(args.hdf5_dir), '--lambda-jepa', str(row['lambda_jepa']),
