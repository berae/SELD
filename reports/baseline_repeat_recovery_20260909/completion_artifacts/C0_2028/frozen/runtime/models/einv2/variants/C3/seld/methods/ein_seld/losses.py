...
class Losses:
    def __init__(self, cfg):
        ...
        self.lambda_velocity = float(cfg['training']['lambda_velocity'])
    ...
    def calculate(self, pred, target, target_latent, epoch_it=0):
        ...
        velocity_elementwise = F.smooth_l1_loss(
            pred['velocity'], updated_target['velocity'], reduction='none').mean(dim=-1)
        velocity_mask = updated_target['velocity_mask']
        loss_velocity = (velocity_elementwise * velocity_mask).sum() / \
            velocity_mask.sum().clamp_min(1.0)
        ...
        loss_all = self.beta * loss_sed + (1 - self.beta) * loss_doa + \
            self.lambda_velocity * loss_velocity + self.lambda_jepa * loss_jepa
    ...
    def tPIT(self, pred, target):
        ...
        target_flipped = {
            ...
            'velocity': target['velocity'].flip(dims=[2]),
            'velocity_mask': target['velocity_mask'].flip(dims=[2])
        }
        ...
        updated_target_velocity = target['velocity'].clone() * (loss1[:, :, None, None] <= loss2[:, :, None, None]) + \
            target_flipped['velocity'].clone() * (loss1[:, :, None, None] > loss2[:, :, None, None])
        updated_target_velocity_mask = target['velocity_mask'].clone() * (loss1[:, :, None] <= loss2[:, :, None]) + \
            target_flipped['velocity_mask'].clone() * (loss1[:, :, None] > loss2[:, :, None])
        updated_target = {
            'sed': updated_target_sed,
            'doa': updated_target_doa,
            'velocity': updated_target_velocity,
            'velocity_mask': updated_target_velocity_mask
        }
