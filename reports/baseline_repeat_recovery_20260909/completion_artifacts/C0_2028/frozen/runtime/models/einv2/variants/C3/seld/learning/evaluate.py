...
def evaluate(cfg, dataset):
    ...
    result_path = out_eval_dir.joinpath('results_dcase2023_micro.csv')
    df = pd.DataFrame(metrics_scores, index=[0])
    df['metric_protocol'] = 'dcase2023_micro_full_duration'
    df.to_csv(result_path, sep=',', mode='w', index=False)
