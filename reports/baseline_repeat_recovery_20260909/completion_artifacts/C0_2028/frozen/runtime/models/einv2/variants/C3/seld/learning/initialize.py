...
def init_train(args, cfg, dataset):
    ...
    if out_train_dir.exists():
        raise FileExistsError('Use a new train_id; preserving existing run: ' + str(out_train_dir))
    ...
    ckptIO = CheckpointIO(
        ...
        extra_modules={'target_model': trainer.target_model}
    )
...
def init_infer(args, cfg, dataset):
    ...
    if out_infer_dir.exists():
        raise FileExistsError('Use a new infer_id; preserving predictions: ' + str(out_infer_dir))
    ...
    ckpts_paths_list = []
    ckpts_models_list = []
    explicit = cfg['inference'].get('checkpoint_paths')
    if explicit:
        ckpts_paths_list = [Path(path) for path in explicit]
        ckpts_models_list = [models[0]] * len(explicit)
    else:
