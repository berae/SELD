...
def main():
    ...
    model = runner.AuditedEINV2(cfg,dataset).to(device).eval()
    ...
    # Match frozen runner's no_grad execution mode; inference_mode can select
    # numerically different kernels near integer-degree rounding boundaries.
    with torch.no_grad():
