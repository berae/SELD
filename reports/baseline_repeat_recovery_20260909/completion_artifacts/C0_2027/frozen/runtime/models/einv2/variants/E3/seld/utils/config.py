...
import methods.feature_causal as feature_causal
...
def get_metrics(cfg, dataset):
    ...
    """
    metrics = method_dict[cfg['method']].metrics.Metrics(cfg, dataset)
...
def get_afextractor(cfg, cuda):
    ...
    elif cfg['data']['audio_feature'] == 'causal_logmel&intensity':
        afextractor = feature_causal.CausalLogmelIntensity_Extractor(cfg)
    else:
        raise ValueError("Unsupported audio feature: {}".format(cfg['data']['audio_feature']))
