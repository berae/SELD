...
class UserDataset(Dataset):
    ...
    def __init__(self, args, cfg, dataset, dataset_type='train', overlap=''):
        ...
        self.jepa_dir = Path(cfg['jepa_hdf5_dir']).joinpath('dev')
        self.velocity_dir = Path(cfg['velocity_hdf5_dir']).joinpath('dev')
        ...
        if self.read_into_mem:
            ...
            for path in iterator:
                ...
                if 'test' not in self.dataset_type:
                    ...
                    jepa_path = self.jepa_dir.joinpath(fn + '.h5')
                    with h5py.File(jepa_path, 'r') as hf:
                        jepa_valid_mask = hf['jepa_valid_mask'][index_begin_label: index_end_label, ...]
                    velocity_path = self.velocity_dir.joinpath(fn + '.h5')
                    with h5py.File(velocity_path, 'r') as hf:
                        velocity_label = hf['velocity_label'][index_begin_label: index_end_label, ...]
                        velocity_mask = hf['velocity_mask'][index_begin_label: index_end_label, ...]
                    if pad_width_after_label != 0:
                        ...
                        jepa_valid_mask_new = np.zeros((pad_width_after_label, 2, 3))
                        velocity_label_new = np.zeros((pad_width_after_label, 2, 3))
                        velocity_mask_new = np.zeros((pad_width_after_label, 2))
                        sed_label = np.concatenate((sed_label, sed_label_new), axis=0)
                        doa_label = np.concatenate((doa_label, doa_label_new), axis=0)
                        jepa_valid_mask = np.concatenate((jepa_valid_mask, jepa_valid_mask_new), axis=0)
                        velocity_label = np.concatenate((velocity_label, velocity_label_new), axis=0)
                        velocity_mask = np.concatenate((velocity_mask, velocity_mask_new), axis=0)
    ...
    def __getitem__(self, idx):
        ...
        if self.read_into_mem:
            ...
            if 'test' not in self.dataset_type:
                ...
                jepa_valid_mask = data_dict['jepa_valid_mask']
                velocity_label = data_dict['velocity_label']
                velocity_mask = data_dict['velocity_mask']
        else:
...
class PinMemCustomBatch:
    def __init__(self, batch_dict):
        ...
        batch_sed_label = []
        batch_doa_label = []
        batch_jepa_valid_mask = []
        batch_velocity_label = []
        batch_velocity_mask = []
        ...
        for n in range(len(batch_dict)):
            ...
            batch_jepa_valid_mask.append(batch_dict[n]['jepa_valid_mask'])
            batch_velocity_label.append(batch_dict[n]['velocity_label'])
            batch_velocity_mask.append(batch_dict[n]['velocity_mask'])
        ...
        self.batch_out_dict = {
            ...
            'jepa_valid_mask': torch.tensor(batch_jepa_valid_mask, dtype=torch.float32),
            'velocity_label': torch.tensor(batch_velocity_label, dtype=torch.float32),
            'velocity_mask': torch.tensor(batch_velocity_mask, dtype=torch.float32),
        }
    ...
    def pin_memory(self):
        ...
        self.batch_out_dict['jepa_valid_mask'] = self.batch_out_dict['jepa_valid_mask'].pin_memory()
        self.batch_out_dict['velocity_label'] = self.batch_out_dict['velocity_label'].pin_memory()
        self.batch_out_dict['velocity_mask'] = self.batch_out_dict['velocity_mask'].pin_memory()
