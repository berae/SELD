...
class Trainer(BaseTrainer):
    ...
    def __init__(self, args, cfg, dataset, af_extractor, valid_set, model, optimizer, losses, metrics):
        ...
        self.ema_momentum = float(cfg['training']['jepa_ema_momentum'])
        ...
        scalar_path = Path(cfg.get('causal_scalar_path') or scalar_h5_dir.joinpath(fn_scalar))
        with h5py.File(scalar_path, 'r') as hf:
            self.mean = hf['mean'][:]
            self.std = hf['std'][:]
        if cfg['data'].get('foa_iv_acn_reorder', False):
            # The cached C0 scalar was calculated for the previous IV order
            # [ch1, ch2, ch3].  Match it to the corrected [ch3, ch1, ch2]
            # feature order without modifying the shared scalar artifact.
            channel_order = [0, 1, 2, 3, 6, 4, 5]
            self.mean = self.mean[:, channel_order]
            self.std = self.std[:, channel_order]
        if args.cuda:
    ...
    def update_ema_target(self):
        for target_parameter, online_parameter in zip(
            ...
            target_parameter.mul_(self.ema_momentum).add_(
                online_parameter.detach(), alpha=1.0 - self.ema_momentum)
        for target_buffer, online_buffer in zip(
                self.target_model.buffers(), self.model.buffers()):
            if target_buffer.is_floating_point():
                target_buffer.mul_(self.ema_momentum).add_(
                    online_buffer.detach(), alpha=1.0 - self.ema_momentum)
            else:
    ...
    def train_step(self, batch_sample, epoch_it):
        ...
        batch_target = {
            ...
            'velocity': batch_sample['velocity_label'],
            'velocity_mask': batch_sample['velocity_mask'],
            'jepa_valid_mask': batch_sample['jepa_valid_mask']
        }
        if self.cuda:
            batch_x = batch_x.cuda(non_blocking=True)
            batch_target['sed'] = batch_target['sed'].cuda(non_blocking=True)
            batch_target['doa'] = batch_target['doa'].cuda(non_blocking=True)
            batch_target['velocity'] = batch_target['velocity'].cuda(non_blocking=True)
            batch_target['velocity_mask'] = batch_target['velocity_mask'].cuda(non_blocking=True)
            batch_target['jepa_valid_mask'] = batch_target['jepa_valid_mask'].cuda(non_blocking=True)
        ...
        with torch.no_grad():
            target_latent = self.target_model(batch_x, latent_only=True)['latent']
        loss_dict = self.losses.calculate(pred, batch_target, target_latent)
        loss_dict[self.cfg['training']['loss_type']].backward()
        self.optimizer.step()
        self.update_ema_target()

        self.train_losses['loss_all'] += loss_dict['all']
        self.train_losses['loss_sed'] += loss_dict['sed']
        self.train_losses['loss_doa'] += loss_dict['doa']
        self.train_losses['loss_velocity'] += loss_dict['velocity']
        self.train_losses['loss_jepa'] += loss_dict['jepa']
    ...
    def validate_step(self, generator=None, max_batch_num=None, valid_type='train', epoch_it=0):
        ...
        if valid_type == 'train':
            ...
            loss_all, loss_sed, loss_doa, loss_velocity, loss_jepa = 0., 0., 0., 0., 0.
            velocity_l2_sum, velocity_l2_count = 0., 0.
            velocity_cosine_sum, velocity_cosine_count = 0., 0.
            horizon_loss_sum = np.zeros(len(self.losses.jepa_horizons), dtype=np.float64)
            latent_count = 0
            latent_sum = None
            latent_second = None
            latent_normalized_sum = None
            ...
            for batch_idx, batch_sample in enumerate(generator):
                ...
                with torch.no_grad():
                    ...
                    pred = self.model(batch_x)
                    target_latent = self.target_model(batch_x, latent_only=True)['latent']
                loss_dict = self.losses.calculate(pred, batch_target, target_latent, epoch_it)
                ...
                loss_velocity += loss_dict['velocity'].cpu().detach().numpy()
                loss_jepa += loss_dict['jepa'].cpu().detach().numpy()
                velocity_mask = loss_dict['updated_target']['velocity_mask'] > 0.5
                velocity_error = torch.linalg.vector_norm(
                    pred['velocity'] - loss_dict['updated_target']['velocity'], dim=-1)
                velocity_l2_sum += velocity_error[velocity_mask].sum().item()
                velocity_l2_count += velocity_mask.sum().item()
                target_norm = torch.linalg.vector_norm(
                    loss_dict['updated_target']['velocity'], dim=-1)
                cosine_mask = velocity_mask & (target_norm > 1e-6)
                if cosine_mask.any():
                    velocity_cosine = torch.nn.functional.cosine_similarity(
                        pred['velocity'], loss_dict['updated_target']['velocity'], dim=-1, eps=1e-8)
                    velocity_cosine_sum += velocity_cosine[cosine_mask].sum().item()
                    velocity_cosine_count += cosine_mask.sum().item()
                horizon_loss_sum += np.asarray([
                    value.detach().cpu().item() for value in loss_dict['jepa_horizon_losses']])
                latent_vectors = loss_dict['jepa_target_vectors'].detach().float().cpu().double()
                if latent_vectors.shape[0] > 0:
                    normalized_vectors = torch.nn.functional.normalize(latent_vectors, dim=-1, eps=1e-12)
                    if latent_sum is None:
                        latent_sum = torch.zeros(latent_vectors.shape[1], dtype=torch.float64)
                        latent_second = torch.zeros(
                            (latent_vectors.shape[1], latent_vectors.shape[1]), dtype=torch.float64)
                        latent_normalized_sum = torch.zeros(latent_vectors.shape[1], dtype=torch.float64)
                    latent_count += latent_vectors.shape[0]
                    latent_sum += latent_vectors.sum(dim=0)
                    latent_second += latent_vectors.T @ latent_vectors
                    latent_normalized_sum += normalized_vectors.sum(dim=0)
            ...
            out_losses = {
                ...
                'loss_velocity': loss_velocity / (batch_idx + 1),
                'loss_jepa': loss_jepa / (batch_idx + 1),
            }
            ...
            metrics_scores['velocity_l2_mae'] = velocity_l2_sum / max(velocity_l2_count, 1)
            metrics_scores['velocity_cosine'] = velocity_cosine_sum / max(velocity_cosine_count, 1)
            for index, frame_offset in enumerate(self.losses.jepa_horizons):
                metrics_scores['jepa_loss_{}ms'.format(frame_offset * 100)] = \
                    horizon_loss_sum[index] / (batch_idx + 1)
            if latent_count == 0:
                raise RuntimeError('No valid same-object JEPA target vectors in validation set')
            latent_mean = latent_sum / latent_count
            covariance = latent_second / max(latent_count, 1) - torch.outer(latent_mean, latent_mean)
            dimension_std = covariance.diag().clamp_min(0).sqrt()
            offdiag = covariance - torch.diag_embed(covariance.diag())
            pair_cosine = (latent_normalized_sum.square().sum() - latent_count) / \
                max(latent_count * (latent_count - 1), 1)
            metrics_scores['target_latent_std_mean'] = float(dimension_std.mean())
            metrics_scores['target_latent_std_min'] = float(dimension_std.min())
            metrics_scores['target_latent_cov_offdiag_abs_mean'] = float(offdiag.abs().mean())
            metrics_scores['target_latent_pair_cosine_mean'] = float(pair_cosine)
