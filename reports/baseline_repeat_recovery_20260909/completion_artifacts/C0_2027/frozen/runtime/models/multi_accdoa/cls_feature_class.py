...
class FeatureClass:
    def __init__(self, params, is_eval=False):
        ...
        self._causal_frontend = params.get('causal_frontend', False)
        self._normalization_fit_splits = params.get('normalization_fit_splits')
    ...
    def _spectrogram(self, audio_input, _nb_frames):
        ...
        for ch_cnt in range(_nb_ch):
            audio_ch = np.asfortranarray(audio_input[:, ch_cnt])
            if self._causal_frontend:
                # Frame t is timestamped at its right edge. Left padding supplies
                # unavailable history, so no sample after that timestamp is read.
                left_padding = self._nfft - self._hop_len
                audio_ch = np.pad(audio_ch, (left_padding, 0), mode='constant')
    ...
    @staticmethod
    def _select_normalization_fit_files(file_names, fit_splits):
        file_names = sorted(file_names)
        if fit_splits is None:
            return file_names

        fit_splits = set(int(split) for split in fit_splits)
        selected = []
        for file_name in file_names:
            match = re.match(r'^fold(\d+)_', file_name)
            if match and int(match.group(1)) in fit_splits:
...
#            pool.join()
    ...
    def preprocess_features(self):
        ...
        if self._is_eval:
            ...
            all_feature_files = [
                file_name for file_name in os.listdir(self._feat_dir)
                if file_name.endswith('.npy')
            ]
            fit_feature_files = self._select_normalization_fit_files(
                all_feature_files, self._normalization_fit_splits
            )
            if not fit_feature_files:
                raise ValueError(
                    'No normalization files matched fit splits {}'.format(
                        self._normalization_fit_splits
                    )
                )

            spec_scaler = preprocessing.StandardScaler()
            for file_cnt, file_name in enumerate(fit_feature_files):
            ...
            with open(normalized_features_wts_file + '.json', 'w') as metadata_file:
                json.dump(
                    {
                        'fit_splits': self._normalization_fit_splits,
                        'fit_file_count': len(fit_feature_files),
                        'fit_files': fit_feature_files,
                    },
                    metadata_file,
                    indent=2,
                    sort_keys=True,
        ...
        for file_cnt, file_name in enumerate(sorted(os.listdir(self._feat_dir))):
