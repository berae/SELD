"""Finite C0-C3 x three-seed queue. Does not retry failures or touch old runs."""
import argparse
...
def main():
    ...
    for seed in (2026, 2027, 2028):
        for variant in ('C0', 'C1', 'C2', 'C3'):
            row = dict(id='{}_seed{}_framecausal_v020'.format(variant, seed), seed=seed, variant=variant, status='pending')
            rows.append(row)
            jobs.put(row)
    lock = threading.Lock()
    ...
    def worker(gpu):
        while not stop.is_set():
            ...
            train = [sys.executable, str(ROOT / 'scripts/train/einv2_audited.py'), '--project-root', str(args.project_root),
                     '--output-root', str(args.experiment_root / 'runs'), '--scalar', str(args.experiment_root / 'scalar_trainfolds2-6.h5'),
                     '--variant', row['variant'], '--seed', str(row['seed'])]
            run = args.experiment_root / 'runs' / row['id']
            commands = [('training', train)] + [(split, [sys.executable, str(ROOT / 'scripts/eval/einv2_audited.py'),
                         '--run', str(run), '--split', split]) for split in ('validation', 'evaluation')]
